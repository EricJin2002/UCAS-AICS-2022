...
class VGG19(object):
    ...
    def load_model(self):
        ...
        for idx in range(self.net.size()):
            if 'conv' in self.net.getLayerName(idx):
                ...
                weight = np.transpose(weight, (3, 2, 0, 1)).reshape(-1).astype(np.float)
            ...
            if 'fc' in self.net.getLayerName(idx):
                ...
                weight = np.reshape(weight, (-1, weight.shape[-1])).transpose().reshape(-1).astype(np.float)
                # weight = np.transpose(weight, (weight.ndim - 1,) + tuple(np.arange(weight.ndim - 1))).reshape(-1).astype(np.float)
                bias = bias.reshape(-1).astype(np.float)
                self.net.loadParams(idx, weight, bias, self.filter_quant_params[count])
                count += 1
    ...
    def load_image(self, image_dir):
        ...
        input_data = input_image.flatten().tolist()
