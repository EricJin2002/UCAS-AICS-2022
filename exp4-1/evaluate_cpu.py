...
def net(data_path, input_image):
    ...
    for i, name in enumerate(layers):
        if name[:4] == 'conv':
            ...
            # print(name[:4],kernels.shape,bias.shape);
            ...
            current = tf.nn.softmax(current, 1)
        elif name  == 'fc6':
            ...
            current = tf.nn.bias_add(
                tf.matmul(
                    tf.reshape(current, (current.shape[0], -1)), 
                    tf.reshape(kernels, (-1, kernels.shape[-1]))
                ),
                bias.flatten()
            )
        elif name  == 'fc7':
...
def _conv_layer(input, weights, bias):
    ...
    return tf.nn.bias_add(
        tf.nn.conv2d(input, weights, 1, 'SAME'),
        bias.flatten()
    )
...
def load_image(path):
    ...
    image = preprocess(image, mean).astype("float32").reshape((1,224,224,3))
    return image
