...
def loss_function(net, content_features, style_features, content_weight, style_weight, tv_weight, preds, batch_size):
    ...
    content_loss = content_weight * tf.nn.l2_loss(net[CONTENT_LAYER] - content_features[CONTENT_LAYER]) * 2 / content_size
    ...
    for style_layer in STYLE_LAYERS:
        ...
        # style_feats = tf.reshape(style_gram, (bs, height * width, filters))
        # style_feats_T = tf.transpose(style_feats, perm=[0,2,1])
        # style_grams = tf.matmul(style_feats_T, style_feats) / size
        # style_losses.append(tf.nn.l2_loss(grams - style_grams) * 2)
        style_losses.append(tf.nn.l2_loss(grams - style_gram) * 2)
    ...
    y_tv = tf.nn.l2_loss(preds[:,1:,:,:]-preds[:,:-1,:,:])
    x_tv = tf.nn.l2_loss(preds[:,:,1:,:]-preds[:,:,:-1,:])
...
def optimize(content_targets, style_target, content_weight, style_weight,
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'), tf.Session() as sess:
        ...
        X_content = tf.placeholder(tf.float32, batch_shape, name="X_content")
        ...
        content_loss, style_loss, tv_loss, loss = loss_function(
            net, content_features, style_features, content_weight, style_weight,
            tv_weight, preds, batch_size
        )
