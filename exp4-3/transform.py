...
def net(image, type=0):
    ...
    print(conv3.shape)
    resd1 = _residual_block(conv3)
    print(resd1.shape)
    resd2 = _residual_block(resd1)
    print(resd2.shape)
    resd3 = _residual_block(resd2)
    print(resd3.shape)
    resd4 = _residual_block(resd3)
    print(resd4.shape)
    resd5 = _residual_block(resd4)
    print(resd5.shape)
    conv4 = _conv_tranpose_layer(resd5, 64, 3, 2)
    print(conv4.shape)
    conv5 = _conv_tranpose_layer(conv4, 32, 3, 2)
    print(conv5.shape)
    conv6 = _conv_layer(conv5, 3, 9, 1)
    print(conv6.shape)

    #TODO：最后一个卷积层的输出再经过 tanh 函数处理，最后的输出张量 preds 像素值需限定在 [0,255] 范围内
    preds = (tf.nn.tanh(conv6) + 1) * 127.5
    return preds
...
def _conv_tranpose_layer(net, num_filters, filter_size, strides, type=0):
    ...
    output_shape = [batch, rows*strides, cols*strides, num_filters]
    strides_shape = [1, strides, strides, 1]

    # TODO：进行转置卷积计算
    net = tf.nn.conv2d_transpose(net, weights_init, output_shape, strides_shape, 'SAME')
...
def _residual_block(net, filter_size=3, type=0):
    ...
    batch, rows, cols, channels = [i.value for i in net.get_shape()]
    weights_init_1 = _conv_init_vars(net, channels, filter_size)
    weights_init_2 = _conv_init_vars(net, channels, filter_size)
    strides_shape = [1, 1, 1, 1]
    x = net
    net = tf.nn.conv2d(net, weights_init_1, strides_shape, "SAME")
    if type == 0:
        net = _batch_norm(net)
    elif type == 1:
        net = _instance_norm(net)
    net = tf.nn.relu(net)
    net = tf.nn.conv2d(net, weights_init_2, strides_shape, "SAME")
    if type == 0:
        net = _batch_norm(net)
    elif type == 1:
        net = _instance_norm(net)
