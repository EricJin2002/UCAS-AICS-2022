...
def run_ori_power_diff_pb(ori_power_diff_pb, image):
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            input_tensor = sess.graph.get_tensor_by_name('X_content:0')
            input_pow_placeholder = sess.graph.get_tensor_by_name('moments_15/PowerDifference_z:0')
            output_tensor = sess.graph.get_tensor_by_name('add_37:0')
            input_pow = np.array(2, dtype=np.float32)

            start_time = time.time()
            ret =sess.run(output_tensor, feed_dict={input_tensor:[X], input_pow_placeholder:input_pow})
            end_time = time.time()
