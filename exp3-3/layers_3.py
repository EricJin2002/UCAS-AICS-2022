...
class ContentLossLayer(object):
    ...
    def forward(self, input_layer, content_layer):
        ...
        loss = np.sum((input_layer - content_layer)**2) / (2.0 * np.size(input_layer))
        return loss
    def backward(self, input_layer, content_layer):
        # TODO： 计算内容损失的反向传播
        bottom_diff = (input_layer - content_layer) / np.size(input_layer)
        return bottom_diff
...
class StyleLossLayer(object):
    ...
    def forward(self, input_layer, style_layer):
        ...
        loss = style_diff / (4.0 * input_layer.shape[0] * self.div)
        return loss
    def backward(self, input_layer, style_layer):
        ...
        for idxn in range(input_layer.shape[0]):
            ...
            bottom_diff[idxn, :, :] = np.matmul(
                (self.gram_input[idxn, :, :] - self.gram_style[idxn, :, :]).T, self.input_layer_reshape[idxn, :, :]
