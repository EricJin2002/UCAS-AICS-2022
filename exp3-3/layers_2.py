...
class MaxPoolingLayer(object):
    ...
    def backward_speedup(self, top_diff):
        ...
        if self.kernel_size == self.stride:
            # 以下方法仅适用于kernel_size == stride的情况，该实验所有maxpooling层均适用
            # 牺牲通用性换取极致性能
            # 下边注释的代码兼容所有情况，但经测试会慢一点
            ...
            return self.backward_raw_book(top_diff)
            ...
            for idxc in range(top_diff.shape[1]):
                for idxh in range(top_diff.shape[2]):
                    for idxw in range(top_diff.shape[3]):
                        ...
                        #     ==1))[:,0] # 当kernel_size > stride时会出错，因为无法从窗口内的多个1中挑选正确index。这需要修改start code代码（self.max_index部分）以改进。对于本实验，不会出现这种例外情况，因此不对初始提供代码做额外修改。
                        
                        # 上述代码弃用，改用下述对所有条件均适用的方法
